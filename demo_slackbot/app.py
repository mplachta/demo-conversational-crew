    ...
    while max_polling_time > 0:
        try:
            status_response = requests.get(
                f"{base_url}/status/{kickoff_id}", headers=headers, timeout=5
            )
...
def submit_message(message, session_id=None):
    ...
    try:
        response = requests.post(
            f"{base_url}/kickoff", json={"inputs": inputs}, headers=headers, timeout=10
        )
...
def handle_mention(event, say, client):
    ...
    try:
        ...
        logger.info(
            f"Processing mention from user {user} in channel {channel}: '{message}'"
        )
        ...
        if result:
            ...
            logger.info(
                f"Sent response to thread {thread_ts}: '{result['response'][:100]}...'"
            )
        else:
            say(
...
def handle_assistant_thread_started(event, say, client, logger):
    ...
    try:
        ...
        if not channel or not thread_ts:
            logger.warning(
                "Missing channel or thread_ts in assistant_thread_started event"
            )
            return
        ...
        logger.info(
            f"Assistant thread started by user {user} in channel {channel}, thread {thread_ts}"
        )
        ...
        logger.error(
            f"Error handling assistant_thread_started: {str(e)}", exc_info=True
        )
...
def handle_assistant_thread_context_changed(event, logger):
    ...
    try:
        ...
        if not channel or not thread_ts:
            logger.debug(
                "Missing channel or thread_ts in assistant_thread_context_changed event"
            )
            return
        ...
        logger.debug(
            f"Assistant thread context changed in channel {channel}, thread {thread_ts}"
        )

    except Exception as e:
        logger.error(
            f"Error handling assistant_thread_context_changed: {str(e)}", exc_info=True
        )
...
def handle_message_events(event, say, client, logger):
    ...
    try:
        ...
        logger.debug(
            f"Received message event: channel={event.get('channel')}, thread_ts={event.get('thread_ts')}, channel_type={event.get('channel_type')}, subtype={event.get('subtype')}, text={event.get('text', '')[:50]}"
        )
        ...
        if channel_type == "im":
            ...
            if result:
                ...
                logger.info(
                    f"Sent DM response to user {user}: '{result['response'][:100]}...'"
                )
            else:
                say(
                    text="Sorry, I encountered an error processing your request. Please try again."
        ...
        logger.debug(
            f"Checking if session_id {session_id} is in active_threads: {session_id in active_threads}. Active threads: {active_threads}"
        )

        if session_id not in active_threads:
            logger.debug(
                f"Session {session_id} not in active threads, ignoring message"
            )
            return

        logger.info(
            f"Processing thread message from user {user} in channel {channel}: '{text}'"
        )
        ...
        if result:
            ...
            logger.info(
                f"Sent thread response to {thread_ts}: '{result['response'][:100]}...'"
            )
        else:
            say(
                text="Sorry, I encountered an error processing your request. Please try again.",
